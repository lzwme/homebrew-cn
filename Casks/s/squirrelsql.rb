cask "squirrelsql" do
  ...
  url "https:downloads.sourceforge.netsquirrel-sql1-stable#{version}squirrel-sql-#{version}-MACOSX-install.jar",
  ...
  postflight do
    system_command "usrbinjava",
                   args: ["-jar", "#{staged_path}squirrel-sql-#{version}-MACOSX-install.jar", installoptions.to_s]
  end

  uninstall_preflight do
    system_command "usrbinjava",
                   args: ["-jar", "#{appdir}SQuirreLSQL.appUninstalleruninstaller.jar", "-f", "-c"]
  end
