cask "temurin" do
  ...
  livecheck do
    url "https:api.adoptium.netv3inforelease_versions?release_type=ga&architecture=#{arch}&image_type=jdk&jvm_impl=hotspot&os=mac&page=0&page_size=1&project=jdk&sort_method=DEFAULT&sort_order=DESC&vendor=eclipse"
    regex(^(\d+(?:\.\d+)*)\+(\d+(?:\.\d+)*)(?:-LTS)?$i)
