cask "font-d2coding" do
  ...
  url "https:github.comnaverd2codingfontreleasesdownloadVER#{version.csv.first}D2Coding-Ver#{version.csv.first}#{"-#{version.csv.second}" if version.csv.second}.zip"
  name "D2 Coding"
  homepage "https:github.comnaverd2codingfont"

  livecheck do
    url :url
    regex(D2Coding[._-](?:Ver|v)?v?(\d+(?:\.\d+)+)(?:-(v?(\d+(?:\.\d+)*)))?\.zipi)
