class Packr < Formula
  ...
  homepage "https:github.comgobuffalopackr"
  url "https:github.comgobuffalopackrarchiverefstagsv2.8.3.tar.gz"
  ...
  test do
    mkdir_p testpath"templatesadmin"

    (testpath"templatesadminindex.html").write <<~HTML
      <!doctype html>
      <html lang="en">
      <head>
        <title>Example<title>
    ...
    (testpath"main.go").write <<~GO
      ...
      import (
        ...
        "github.comgobuffalopackrv2"
      )

      func main() {
        box := packr.New("myBox", ".templates")

        s, err := box.FindString("adminindex.html")
        if err != nil {
          log.Fatal(err)
        }
    ...
    system "go", "mod", "edit", "-require=github.comgobuffalopackrv2@v#{version}"
    system "go", "mod", "tidy"
    ...
    assert_equal File.read("templatesadminindex.html"), shell_output(".example")
  end
end