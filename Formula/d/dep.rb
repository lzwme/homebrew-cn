class Dep < Formula
  ...
  homepage "https:github.comgolangdep"
  url "https:github.comgolangdep.git",
      tag:      "v0.5.4",
      revision: "1f7c19e5f52f49ffb9f956f64c010be14683468b"
  license "BSD-3-Clause"
  head "https:github.comgolangdep.git", branch: "master"
  ...
  # Original deprecation date: 2020-11-25
  # Temporarily undeprecated from 2022-07-29 to 2022-09-20
  disable! date: "2022-12-20", because: :repo_archived
  ...
  def install
    ...
    (buildpath"srcgithub.comgolangdep").install buildpath.children
    cd "srcgithub.comgolangdep" do
      ENV["DEP_BUILD_PLATFORMS"] = platform
      ENV["DEP_BUILD_ARCHS"] = arch
      system "hackbuild-all.bash"
      bin.install "releasedep-#{platform}-#{arch}" => "dep"
      prefix.install_metafiles
    end
  end

  test do
    # Default HOMEBREW_TEMP is tmp, which is actually a symlink to privatetmp.
    # `dep` bails without `.realpath` as it expects $GOPATH to be a "real" path.
    ENV["GOPATH"] = testpath.realpath
    project = testpath"srcgithub.comprojecttesting"
    (project"hello.go").write <<~EOS
      package main

      import "fmt"
      import "github.comMastermindsvcs"
    ...
    cd project do
      ...
      assert_match %r{github.comMastermindsvcs\s+1.11.0\s}, output
    end
  end
end

__END__
diff --git ahackbuild-all.bash bhackbuild-all.bash
