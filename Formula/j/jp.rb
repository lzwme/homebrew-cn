class Jp < Formula
  ...
  homepage "https:github.comsgrebenjp"
  ...
  def install
    ...
    build_root = buildpath"srcgithub.comsgrebenjp"
...
--- aMakefile
+++ bMakefile
@@ -90,3 +90,10 @@ release$(APP)_$(VERSION)_linux_arm64.zip: binarieslinux_arm64$(APP)

 binarieslinux_arm64$(APP): $(GOFILES)
 	GOOS=linux GOARCH=arm64 go build -ldflags "-X main.version=$(VERSION)" -o binarieslinux_arm64$(APP) .cmd$(APP)
+
+release$(APP)_$(VERSION)_osx_arm64.zip: binariesosx_arm64$(APP)
+	mkdir -p release
+	cd .binariesosx_arm64 && zip -r -D ....release$(APP)_$(VERSION)_osx_arm64.zip $(APP)
+
+binariesosx_arm64$(APP): $(GOFILES)
+	GOOS=darwin GOARCH=arm64 go build -ldflags "-X main.version=$(VERSION)" -o binariesosx_arm64$(APP) .cmd$(APP)