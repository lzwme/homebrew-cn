class Govendor < Formula
  ...
  homepage "https:github.comkardianosgovendor"
  url "https:github.comkardianosgovendorarchiverefstagsv1.0.9.tar.gz"
  sha256 "d303abf194838792234a1451c3a1e87885d1b2cd21774867b592c1f7db00551e"
  license "BSD-3-Clause"
  head "https:github.comkardianosgovendor.git", branch: "master"
  ...
  def install
    ...
    (buildpath"srcgithub.comkardianos").mkpath
    ln_sf buildpath, buildpath"srcgithub.comkardianosgovendor"
  ...
  test do
    ...
    (testpath"srcgithub.comprojecttesting").mkpath

    cd "srcgithub.comprojecttesting" do
      system bin"govendor", "init"
      assert_predicate Pathname.pwd"vendor", :exist?, "Failed to init!"
      system bin"govendor", "fetch", "-tree", "golang.orgxcrypto@#{commit}"
      assert_match commit, File.read("vendorvendor.json")
      assert_match "golang.orgxcryptoblowfish", shell_output("#{bin}govendor list")
    end
  end
end