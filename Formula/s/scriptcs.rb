class Scriptcs < Formula
  ...
  homepage "https:github.comscriptcsscriptcs"
  url "https:github.comscriptcsscriptcsarchiverefstagsv0.17.1.tar.gz"
  ...
  def install
    ...
    libexec.install Dir["srcScriptCsbinRelease*"]
    (libexecscript_file).write <<~EOS
      #!binbash
      mono #{libexec}scriptcs.exe $@
    EOS
    (libexecscript_file).chmod 0755
    bin.install_symlink libexecscript_file => "scriptcs"
  end

  test do
    test_file = "tests.csx"
    (testpathtest_file).write('Console.WriteLine("{0}, {1}!", "Hello", "world");')
