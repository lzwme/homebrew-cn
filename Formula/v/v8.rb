class V8 < Formula
  ...
  homepage "https:v8.devdocs"
  # Track V8 version from Chrome stable: https:chromiumdash.appspot.comreleases?platform=Mac
  # Check `brew livecheck --resources v8` for any resource updates
  url "https:github.comv8v8archiverefstags13.5.212.10.tar.gz"
  sha256 "2c47b3de22591f4384f75aa462301eb9fdb9d661ee07f73cab95a89d0a75d5ae"
  license "BSD-3-Clause"

  livecheck do
    url "https:chromiumdash.appspot.comfetch_releases?channel=Stable&platform=Mac"
    regex((\d+\.\d+\.\d+\.\d+)i)
    strategy :json do |json, regex|
      ...
      v8_page = Homebrew::Livecheck::Strategy.page_content(
        "https:chromium.googlesource.comv8v8.git+#{v8_hash}",
      )
  ...
  # e.g. for CIPD dependency gn: https:chromium.googlesource.comv8v8.git+refstags<version>DEPS#74
  resource "gn" do
    ...
    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(["']gn_version["']:\s*["']git_revision:([0-9a-f]+)["']i)
    end
  end

  resource "build" do
    url "https:chromium.googlesource.comchromiumsrcbuild.git",
        revision: "efb3303345a5501074564393470197a904b4afb7"
    version "efb3303345a5501074564393470197a904b4afb7"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']chromiumsrcbuild\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "buildtools" do
    url "https:chromium.googlesource.comchromiumsrcbuildtools.git",
        revision: "b248db940ef3dd7e5f4694ebf4d8a3f67aa0086d"
    version "b248db940ef3dd7e5f4694ebf4d8a3f67aa0086d"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']chromiumsrcbuildtools\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "third_partyabseil-cpp" do
    url "https:chromium.googlesource.comchromiumsrcthird_partyabseil-cpp.git",
        revision: "2705c6655c0008cc3fb152dae27890d44bc335f1"
    version "2705c6655c0008cc3fb152dae27890d44bc335f1"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']chromiumsrcthird_partyabseil-cpp\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "third_partyfast_floatsrc" do
    url "https:chromium.googlesource.comexternalgithub.comfastfloatfast_float.git",
        revision: "cb1d42aaa1e14b09e1452cfdef373d051b8c02a4"
    version "cb1d42aaa1e14b09e1452cfdef373d051b8c02a4"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']externalgithub.comfastfloatfast_float\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "third_partyfp16src" do
    url "https:chromium.googlesource.comexternalgithub.comMaratyszczaFP16.git",
        revision: "0a92994d729ff76a58f692d3028ca1b64b145d91"
    version "0a92994d729ff76a58f692d3028ca1b64b145d91"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']externalgithub.comMaratyszczaFP16\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "third_partygoogletestsrc" do
    url "https:chromium.googlesource.comexternalgithub.comgooglegoogletest.git",
        revision: "24a9e940d481f992ba852599c78bb2217362847b"
    version "24a9e940d481f992ba852599c78bb2217362847b"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']externalgithub.comgooglegoogletest\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "third_partyhighwaysrc" do
    url "https:chromium.googlesource.comexternalgithub.comgooglehighway.git",
        revision: "00fe003dac355b979f36157f9407c7c46448958e"
    version "00fe003dac355b979f36157f9407c7c46448958e"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']externalgithub.comgooglehighway\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "third_partyicu" do
    ...
    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']chromiumdepsicu\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "third_partyjinja2" do
    url "https:chromium.googlesource.comchromiumsrcthird_partyjinja2.git",
        revision: "5e1ee241ab04b38889f8d517f2da8b3df7cfbd9a"
    version "5e1ee241ab04b38889f8d517f2da8b3df7cfbd9a"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']chromiumsrcthird_partyjinja2\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "third_partymarkupsafe" do
    url "https:chromium.googlesource.comchromiumsrcthird_partymarkupsafe.git",
        revision: "9f8efc8637f847ab1ba984212598e6fb9cf1b3d4"
    version "9f8efc8637f847ab1ba984212598e6fb9cf1b3d4"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']chromiumsrcthird_partymarkupsafe\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "third_partypartition_alloc" do
    url "https:chromium.googlesource.comchromiumsrcbaseallocatorpartition_allocator.git",
  ...
  resource "third_partysimdutf" do
    url "https:chromium.googlesource.comchromiumsrcthird_partysimdutf.git",
        revision: "5a9a2134b280c1b956ad68a0643797fe26dd1c94"
    version "5a9a2134b280c1b956ad68a0643797fe26dd1c94"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']chromiumsrcthird_partysimdutf["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  resource "third_partyzlib" do
    url "https:chromium.googlesource.comchromiumsrcthird_partyzlib.git",
        revision: "788cb3c270e8700b425c7bdca1f9ce6b0c1400a9"
    version "788cb3c270e8700b425c7bdca1f9ce6b0c1400a9"

    livecheck do
      url "https:raw.githubusercontent.comv8v8refstags#{LATEST_VERSION}DEPS"
      regex(%r{["']chromiumsrcthird_partyzlib\.git["']\s*\+\s*["']@["']\s*\+\s*["']([0-9a-f]+)["']}i)
    end
  end

  def install
    resources.each { |r| r.stage(buildpathr.name) }
    ...
    ENV.prepend_path "PATH", buildpath"gnout"

    # create gclient_args.gni
    (buildpath"buildconfiggclient_args.gni").write <<~GN
      declare_args() {
    ...
    if OS.linux?
      ...
      gn_args[:custom_toolchain] = "\"buildtoolchainlinuxunbundle:default\"" # uses system toolchain
      gn_args[:host_toolchain] = "\"buildtoolchainlinuxunbundle:default\"" # to respect passed LDFLAGS
      ...
      if DevelopmentTools.clang_build_version <= 1400
        ...
        inreplace "buildconfigmacBUILD.gn", "[ \"-Wl,-ObjC\" ]",
    ...
    libexec.install "out.gnd8", "out.gnicudtl.dat"
  ...
  test do
    ...
    (testpath"test.cpp").write <<~CPP
      #include <libplatformlibplatform.h>
      #include <v8.h>
      int main(){
