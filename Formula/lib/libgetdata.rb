class Libgetdata < Formula
  ...
  url "https:downloads.sourceforge.netprojectgetdatagetdata0.11.0getdata-0.11.0.tar.xz"
  ...
  def install
    ...
    ENV.deparallelize # can't open file: .libslibgetdatabzip2-0.11.0.so (No such file or directory)
    system "make"
    # The Makefile seems to try to install things in the wrong order.
    # Remove this when the following PR is mergedresolved and lands in a release:
    # https:github.comketiltroutgetdatapull6
    system "make", "-C", "bindingsperl", "install-nodist_perlautogetdataSCRIPTS"
    system "make", "install"
  end
