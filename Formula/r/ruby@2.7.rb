class RubyAT27 < Formula
  ...
  url "https:cache.ruby-lang.orgpubruby2.7ruby-2.7.8.tar.xz"
  ...
  resource "digest" do
    url "https:github.comrubydigestarchiverefstagsv3.1.1.tar.gz"
    ...
    %w[openssl digest].each do |r_name|
      resource(r_name).stage do
        ...
        Pathname.new("lib").each_child do |child|
          if child.directory?
            child.each_child { |grandchild| (buildpath"ext"r_namechild).install grandchild }
            next
          end
