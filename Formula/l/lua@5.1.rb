class LuaAT51 < Formula
  ...
  url "https:www.lua.orgftplua-5.1.5.tar.gz"
  mirror "https:deb.debian.orgdebianpoolmainllua5.1lua5.1_5.1.5.orig.tar.gz"
  ...
  on_macos do
    ...
    # See: https:github.comHomebrewhomebrewpull5043
    patch :DATA
  end
  ...
  on_linux do
    ...
    patch do
      url "https:gist.githubusercontent.comiMichka0f389e65e5abd63bfc6073bfa76082b0raw6e9c4c4690c737d93a376e053bcb82cdd69aac3blua5.1.5.patch"
  ...
  def install
    ...
    # Fix paths in the .pc
    inreplace "etclua.pc" do |s|
      s.gsub! "prefix= usrlocal", "prefix=#{HOMEBREW_PREFIX}"
      s.gsub! "INSTALL_MAN= ${prefix}manman1", "INSTALL_MAN= ${prefix}sharemanman1"
      s.gsub! "INSTALL_INC= ${prefix}include", "INSTALL_INC= ${prefix}includelua-5.1"
      s.gsub! "includedir=${prefix}include", "includedir=${prefix}includelua-5.1"
    ...
    (lib"pkgconfig").install "etclua.pc"
    ...
    mv lib"pkgconfiglua.pc", lib"pkgconfiglua-5.1.pc"
    ...
    (libexec"libpkgconfig").install_symlink lib"pkgconfiglua-5.1.pc" => "lua.pc"
...
@@ -43,7 +43,7 @@ PLATS= aix ansi bsd freebsd generic linux macosx mingw posix solaris
 # What to install.
 TO_BIN= lua luac
 TO_INC= lua.h luaconf.h lualib.h lauxlib.h ..etclua.hpp
-TO_LIB= liblua.a
...
-	$(RANLIB) $@
+	$(CC) -dynamiclib -install_name @LUA_PREFIX@libliblua.5.1.dylib \
