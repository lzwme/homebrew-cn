class PhpAT56Zts < Formula
  ...
  url "https:github.comshivammathurphp-src-backportsarchive2caa81b25793a7c1878530ed80a289b070cfa44f.tar.gz"
  version "5.6.40"
  ...
  def caveats
    <<~EOS
      To enable PHP in Apache add the following to httpd.conf and restart Apache:
          LoadModule php5_module #{opt_lib}httpdmoduleslibphp5.so
  ...
  test do
    ...
    begin
      ...
      (testpath"httpd.conf").write <<~EOS
        ...
        LoadModule php5_module #{lib}httpdmoduleslibphp5.so
        <FilesMatch \\.(php|phar)$>
          SetHandler applicationx-httpd-php
        <FilesMatch>
      EOS
...
+       test -f $ICONV_DIR$PHP_LIBDIRlib$iconv_lib_name.tbd
     then
       PHP_CHECK_LIBRARY($iconv_lib_name, libiconv, [
         found_iconv=yes
diff --git aZendzend_compile.h bZendzend_compile.h
...
diff --git aZendzend_execute.h bZendzend_execute.h
